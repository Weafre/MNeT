...
Depending on the environment variable COMPILE_CUDA, compiles the torchdac_backend with or
without support for CUDA, into a module called torchdac_backend_gpu or torchdac_backend_cpu.
...
The only difference between the CUDA and non-CUDA versions is: With CUDA, _get_uint16_cdf from torchdac is done with a
simple/non-optimized CUDA kernel (torchdac_kernel.cu), which has one benefit: we can directly write into shared memory!
...
MODULE_BASE_NAME = 'torchdac_backend'
...
def get_extension(cuda_support):
    ...
    if cuda_support:
        ...
        return CUDAExtension(
                ...
                prefixed(prefix, ['torchdac.cpp', 'torchdac_kernel.cu']),
                define_macros=[('COMPILE_CUDA', '1')])
    else:
        return CppExtension(
                MODULE_BASE_NAME + '_cpu',
                prefixed(prefix, ['torchdac.cpp']))
