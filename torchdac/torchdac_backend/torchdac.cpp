...
#ifdef COMPILE_CUDA
/// All these are defined in torchdac_kernel.cu
...
const struct cdf_ptr get_cdf_ptr(const at::Tensor& cdf)
{
    ...
    TORCH_CHECK(s.size() == 4 && s[0] == 1, "Invalid size for cdf! Expected 1HWLp  sdf")
...
at::Tensor decode(
    ...
    const int precision = 16;  // TODO: unify with torchdac_kernel.cu
