...
from DataPreprocessing.pc_2_blocks import partition_octree
import numpy as np
...
def process(path, args):
    ...
    for i, block in enumerate(blocks):
        ...
        dtypes = [np.int32,np.int32,np.int32,np.uint8,np.uint8,np.uint8]

        pc.to_file(final_target_path)
        cur_pc = arr_to_pc(block, cols, dtypes)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        ...
        description='Partition point clouds into smaller blocks',
    ...
    parser.add_argument('--n', help='Number of largest files to keep.', type=float, default=1.)
    ...
    if (args.n < 1):
        selection = int(args.n * files_len)
    else:
        selection = int(args.n)

    files,_,_=zip(*files_sorted_by_size[:selection])
